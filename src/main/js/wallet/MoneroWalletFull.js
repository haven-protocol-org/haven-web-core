    ...
    return that._module.queueTask(async function() {
      if (isEnabled) {
        that._fullListenerHandle = that._module.set_listener(
            ...
            function(height, startHeight, endHeight, percentDone, message) { that._fullListener.onSyncProgress(height, startHeight, endHeight, percentDone, message); },
            function(height) { that._wasmListener.onNewBlock(height); },
            function(newBalanceStr, newUnlockedBalanceStr, assetType) { that._fullListener.onBalancesChanged(newBalanceStr, newUnlockedBalanceStr, assetType); },
            function(height, txHash, amountStr, assetType, accountIdx, subaddressIdx, version, unlockHeight, isLocked) { that._fullListener.onOutputReceived(height, txHash, amountStr, assetType, accountIdx, subaddressIdx, version, unlockHeight, isLocked); },
            function(height, txHash, amountStr, assetType, accountIdx, subaddressIdx, version, unlockHeight, isLocked) { that._fullListener.onOutputReceived(height, txHash, amountStr, assetType, accountIdx, subaddressIdx, version, unlockHeight, isLocked); },
        )} else {
